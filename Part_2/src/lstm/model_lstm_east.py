...
Oct. 22th: change name to model_lstm_east
"""
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string("prev_checkpoint_path", "/work/cascades/lxiaol9/ARC/EAST/checkpoints/east/20180921-135717/", 'path' )
    # Model running infos
    ...
    flags.DEFINE_string("geometry", 'RBOX', "geometry for bbox")

    FLAGS = flags.FLAGS
    if platform.uname()[1] != 'dragonx-H97N-WIFI':
        print("Now it knows it's in a remote cluster")
        FLAGS.system = "remote"
        FLAGS.data_path = "/work/cascades/lxiaol9/ARC/EAST/data/ICDAR2013/train"
        FLAGS.vis_path = "/home/lxiaol9/ARC/EASTRNN/vis/LSTM/"
        FLAGS.prev_checkpoint_path = "/work/cascades/lxiaol9/ARC/EAST/checkpoints/LSTM_east/20180924-191410/"
        FLAGS.save_path = "/work/cascades/lxiaol9/ARC/EAST/checkpoints/LSTM_east/" + now.strftime("%Y%m%d-%H%M%S") + '/'
        FLAGS.video_path = "/work/cascades/lxiaol9/ARC/EAST/data/ICDAR2013/train"
    ...
    with tf.name_scope("Val"):
        ...
        mvalid = ArrayModel(False, eval_config, x_val, reuse_variables=True, initializer=initializer)
    ...
    with sv.managed_session(config=config_proto) as session:
        if FLAGS.restore:
            ...
            ckpt = FLAGS.prev_checkpoint_path + '-11003'
        ...
        # initialize the last layer with pre-trained weights
        ...
        # w1 = g.get_tensor_by_name('feature_fusion/Conv_7/weights:0')
        ...
        # tf.assign(v1[0], w1) # won't work because you will add ops to the graph
            v1[0].load(wt, session)
            print(session.run(v1[0]))
        v2 = [var for var in tf.trainable_variables() if var.op.name==variable_names[5]]
        print(session.run(v2[0]))
