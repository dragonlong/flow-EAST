...
class ArrayModel(object):
    ...
    def __init__(self, is_training, config, input_data, reuse_variables=None, initializer=None):
        ...
        self.geometry_set = []
        self.statenorm_set = []
        self.featurenorm_set = []
        state = None
        feature_prev = None
        reuse_variables = reuse_variables
        for k in range(config.num_steps):
            F_score, F_geometry, state, feature_prev = self._cnn_lstm(input_data[:, k, :, :, :], state, feature_prev, self.sum_kernel, config, reuse_variables, lstm=True)
            self.score_map_set.append(F_score)
            self.geometry_set.append(F_geometry)
            self.statenorm_set.append(tf.norm(state,axis=-1))
            self.featurenorm_set.append(tf.norm(feature_prev,axis=-1))
