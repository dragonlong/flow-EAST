...
import _init_paths
from pwc.model_base import ModelBase
from pwc.optflow import flow_write, flow_write_as_png, flow_mag_stats
from pwc.losses import pwcnet_loss
from pwc.logger import OptFlowTBLogger
from pwc.multi_gpus import assign_to_device, average_gradients
from pwc.core_warp import dense_image_warp
from pwc.core_costvol import cost_volume
from pwc.utils import tf_where
...
class ModelPWCNet(ModelBase):
    ...
    def predict_from_img_batch_pairs(self, img_pairs, batch_size=1, verbose=False):
            """Inference loop. Run inference on a list of image pairs.
            Args:
                img_pairs: list of image pairs/tuples in list((img_1, img_2),...,(img_n, img_nplusone)) format.
                batch_size: size of the batch to process (all images must have the same dimension, if batch_size>1)
