...
#
# tf.app.flags.DEFINE_boolean("debug", False, "whether to visualize data")
...
def crop_all_random_seq(config, data, score_maps, geo_maps, training_masks, crop_background=False, max_tries=20):
    ...
    if frame_height < config.input_size:
        ...
        tmasks_new = np.ones((config.num_steps, config.input_size, config.input_size), dtype=np.uint8)
        x = 0
...
if __name__ == '__main__':
    ...
    print("Now we're starting data loading!!!")
    time_total = 0
    ...
    average_time = (time.time() - time_start)/100
    print("the average data loading time is {} seonds".format(average_time))
