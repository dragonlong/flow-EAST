...
def generator_fast(input_size=512, batch_size=32,
    ...
    for t in range(ctrl_cnt):
        ...
        index2 = index1
        np.random.shuffle(index1)
        index_double = np.concatenate((index1, index2), axis=0)
        images = []
        score_maps = []
        geo_maps = []
        training_masks = []
...
if __name__ == '__main__':
    ...
    images, score_maps, geo_maps, training_masks = generator_fast()
    print("Creating data!!")
    # batch_size = 4

    # data_generator = get_batch(num_workers=FLAGS.num_readers,
    #                                          input_size=FLAGS.input_size,
    ...
    #     start = time.time()
    #     data = next(data_generator)
    #     print("Using {} seconds loading {} batch-size data".format(time.time()-start, batch_size))
