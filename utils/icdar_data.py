...
def generator_fast(config, input_size=512, batch_size=32,
              background_ratio=3./8,
    ...
    while True:
        ...
        for i in index_double:
            try:
                data = np.zeros([config.num_steps, config.shape[0], config.shape[1], 3], dtype=np.float32)
                score_maps = np.zeros([config.num_steps, int(config.shape[0]/4), int(config.shape[0]/4), 1], dtype=np.float32)
                geo_maps = np.zeros([config.num_steps, int(config.shape[0]/4), int(config.shape[0]/4), 1], dtype=np.float32)
                training_masks = np.zeros([config.num_steps, int(config.shape[0]/4), int(config.shape[0]/4), 1], dtype=np.float32)
...
def generator_fast_sequence(config=None,
              background_ratio=3./8,
              random_scale=np.array([0.5, 1, 1.5, 2.0]),
              vis=False):
    video_set = []
    ...
    for k in index:
        ...
        if FLAGS.from_source is True:
            for m in range(cnt_frame):
                basename = '/work/cascades/lxiaol9/ARC/EAST/data/pre-processed'
                score_name = basename + '/score_maps/' + '{}'.format(video_set[k])+ '/frame' + '{:03d}'.format(m) + '.npy'
                geo_name = basename + '/geo_maps/' + '{}'.format(video_set[k])+ '/frame' + '{:03d}'.format(m) +'.npy'
                mask_name = basename + '/training_masks/' + '{}'.format(video_set[k])+ '/frame' + '{:03d}'.format(m) + '.npy'
                if not os.path.exists(basename + '/score_maps/' + '{}'.format(video_set[k])):
                    os.makedirs(basename + '/score_maps/' + '{}'.format(video_set[k]))
                    os.makedirs(basename + '/geo_maps/' + '{}'.format(video_set[k]))
                    os.makedirs(basename + '/training_masks/' + '{}'.format(video_set[k]))
                # text_polys, text_tags = load_annoataion(txt_fn)
                text_polys, text_tags = polys_array_list[m], tags_array_list[m]
                text_polys, text_tags = check_and_validate_polys(text_polys, text_tags, (frame_height, frame_width))
                if text_polys.shape[0] == 0:
                    score_map = np.zeros((frame_height, frame_width), dtype=np.uint8)
                    geo_map = np.zeros((frame_height, frame_width, 5), dtype=np.float32)
                    training_mask= np.zeros((frame_height, frame_width), dtype=np.uint8)
