...
from pwc.net_options import cfg
...
tf.app.flags.DEFINE_string('data_path', '/work/cascades/lxiaol9/ARC/EAST/data/ICDAR2013/train', 'data of ICDAR')
...
tf.app.flags.DEFINE_string("prev_checkpoint_path", "/work/cascades/lxiaol9/ARC/EAST/checkpoints/FLOW_east/20180921-135717/", 'path' )
...
# here we are running the PWC-Net-sm model
...
def main(argv=None):
    ...
    input_images = tf.placeholder(tf.float32, shape=[40, 512, 512, 3], name='input_images')
    input_flow_maps = tf.placeholder(tf.float32, shape=[40, 128, 128, 2], name='input_flow_maps')
    input_score_maps = tf.placeholder(tf.float32, shape=[8, 128, 128, 1], name='input_score_maps')
    if FLAGS.geometry == 'RBOX':
        input_geo_maps = tf.placeholder(tf.float32, shape=[8, 128, 128, 5], name='input_geo_maps')
    else:
        input_geo_maps = tf.placeholder(tf.float32, shape=[8, 128, 128, 8], name='input_geo_maps')
    input_training_masks = tf.placeholder(tf.float32, shape=[8, 128, 128, 1], name='input_training_masks')
    ...
    with g.as_default():
        ...
        if FLAGS.restore:
            ...
            ckpt = FLAGS.prev_checkpoint_path + '/model.ckpt-19801'
    ...
    for step in range(FLAGS.max_steps):
        ...
        flow_maps = y_hats[:, 1::4, 1::4, :]/4
        # display_img_pairs_w_flows(img_pairs, pred_labels)
        with g.as_default():
            ml, tl, _ = sess1.run([model_loss, total_loss, train_op], feed_dict={input_images: east_feed,
...
def tower_loss(images, flow_maps, score_maps, geo_maps, training_masks, reuse_variables=None):
    """
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
        feature = model_flow_east.model(images, is_training=True)# feature [batch_size/GPU*5, H, W, C]
        feature = tf.stop_gradient(feature)
        indices = tf.constant([[2], [2], [2], [2], [2], [7], [7], [7], [7], [7], [12], [12], [12], [12], [12], [17], [17], [17], [17], [17]])
